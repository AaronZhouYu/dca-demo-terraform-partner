resource "aws_vpc" "chef-vpc" {
  ...
  tags = {
    Name          = "chef_servers_${random_id.instance_id.hex}_vpc"
    X-Dept        = "${var.tag_dept}"
...
resource "aws_internet_gateway" "chef-gateway" {
  ...
  tags = {
    Name          = "chef_servers_${random_id.instance_id.hex}_gateway"
    X-Dept        = "${var.tag_dept}"
    X-Customer    = "${var.tag_customer}"
    X-Project     = "${var.tag_project}"
    X-Contact     = "${var.tag_contact}"
...
resource "aws_route_table" "chef-route-table" {
  ...
  tags = {
    Name = "chef_servers_${random_id.instance_id.hex}_route_table"
  }
}
...
resource "aws_security_group" "chef_servers" {
  name        = "chef_servers_${random_id.instance_id.hex}"
  description = "Chef Servers"
  vpc_id      = aws_vpc.chef-vpc.id

  tags = {
    Name          = "chef_servers_${random_id.instance_id.hex}_security_group"
    X-Dept        = "${var.tag_dept}"
    X-Customer    = "${var.tag_customer}"
    X-Project     = "${var.tag_project}"
    X-Application = "${var.tag_application}"
