# Policyfile.rb - Describe how you want Chef to build your system.
#
# For more information on the Policyfile feature, visit
# https://docs.chef.io/policyfile.html

# A name that describes what the system you're building with Chef does.
name 'prod_windows'

# Where to find external cookbooks:
default_source :supermarket, 'https://supermarket.chef.io'
default_source :chef_repo, '../cookbooks'

# run_list: chef-client will run these recipes in the order specified.
run_list 'chef_client::default', 'windows-hardening::default'

# Specify a custom source for a single cookbook:
# cookbook 'example_cookbook', path: '../cookbooks/example_cookbook'
cookbook 'chef_client', path: '../cookbooks/chef_client'
cookbook 'windows-hardening', path: '../cookbooks/windows-hardening'

# Invoke the Compliance Phase
default['audit']['compliance_phase'] = true

# Set profile locations
default['audit']['profiles']['windows-baseline'] = {
  'compliance': 'admin/windows-baseline'
}

default['audit']['profiles']['cis-windows2019-level1'] = {
  'compliance': 'admin/cis-windows2019-level1-memberserver'
}

default['audit']['profiles']['cis-windows2019-level2'] = {
  'compliance': 'admin/cis-windows2019-level2-memberserver'
}

# Allow 100 results per control
default['audit']['control_results_limit'] = 100

# Fetch additional profiles
default['audit']['fetcher'] = 'chef-server'

# Set reporter
default['audit']['reporter'] = 'chef-server-automate'

# Set independent Compliance Phase scans
default['audit']['interval']['enabled'] = true

# Define the timing of independent Compliance Phase scans
# Sets scan every 3 minutes
default['audit']['interval']['time'] = 5
